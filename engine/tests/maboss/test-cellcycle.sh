...
if [ "$MULTI_THREAD_ONLY" = "" ]; then
    $LAUNCHER $MABOSS cellcycle/cellcycle.bnd -c cellcycle/cellcycle_runcfg.cfg -c cellcycle/cellcycle_runcfg-thread_1.cfg -o tmp/Cell_cycle_thread_1 $FINAL $EXTRA_ARGS
    if [ $? != 0 ]; then exit 1; fi
    ...
    ${PYTHON:-python} compare_fixpoints.py cellcycle/refer/Cell_cycle_thread_1_fp.csv tmp/Cell_cycle_thread_1_fp.csv
...
$LAUNCHER $MABOSS cellcycle/cellcycle.bnd -c cellcycle/cellcycle_runcfg.cfg -c cellcycle/cellcycle_runcfg-thread_6.cfg -o tmp/Cell_cycle_thread_6 $FINAL $EXTRA_ARGS
...
if [[ -z "$FINAL" ]]; then
    ...
    ${PYTHON:-python} compare_fixpoints.py cellcycle/refer/Cell_cycle_thread_6_fp.csv tmp/Cell_cycle_thread_6_fp.csv
    check_file "fixpoints"
